...
def main(args):
  ...
  # device = torch.device('cpu')
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS):
        ###
        train_one_epoch(cfg, model, optimizer, train_loader, device, epoch, tfboard, softmax_criterion_s2, softmax_criterion_s3, output_dir)   ####开始训练
        lr_scheduler.step()

        if (epoch + 1) % cfg.EVAL_PERIOD == 0 or epoch == cfg.SOLVER.MAX_EPOCHS - 1:  #训练多少次测试一次
            evaluate_performance(
                model,
                gallery_loader,
                query_loader,
