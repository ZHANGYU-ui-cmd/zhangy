...
def eval_detection(
    gallery_dataset, gallery_dets, det_thresh=0.5, iou_thresh=0.5, labeled_only=False, outsys_dir = None):
    """
    ...
    annos = gallery_dataset.annotations  ###list为6978个，每个里面有{'img_name', 'img_path', 'boxes', 'pids'}
    ...
    for anno, det in zip(annos, gallery_dets):
        ...
        if det != []:
            det = np.asarray(det)
            inds = np.where(det[:, 4].ravel() >= det_thresh)[0]  #det中scores得分大于0.01的索引
            det = det[inds]
            num_det = det.shape[0]
        else:
            num_det = 0
        ...
        ious = np.zeros((num_gt, num_det), dtype=np.float32)  ##全0数组
    ...
    if outsys_dir is not None:
        write_text(sentence="{} detection:".format("labeled only" if labeled_only else "all"),
                   fpath=osp.join(outsys_dir, 'os.txt'))
        write_text(sentence="  recall = {:.2%}".format(det_rate), fpath=osp.join(outsys_dir, 'os.txt'))

        if not labeled_only:
            write_text(sentence="  ap = {:.2%}".format(ap), fpath=osp.join(outsys_dir, 'os.txt'))
...
def eval_search_cuhk(
    ...
    if outsys_dir is not None:
        write_text(sentence = "search ranking:", fpath = osp.join(outsys_dir, 'os.txt'))
        write_text(sentence = "  mAP = {:.2%}".format(np.mean(aps)), fpath = osp.join(outsys_dir, 'os.txt'))
        for i, k in enumerate(topk):
            write_text("  top-{:2d} = {:.2%}".format(k, accs[i]), fpath = osp.join(outsys_dir, 'os.txt'))
...
def eval_search_prw(
    ...
    # print("search ranking:")
    mAP = np.mean(aps)
    write_text(sentence="  mAP = {:.2%}".format(mAP), fpath=osp.join(outsys_dir, 'os.txt'))
    # print("  mAP = {:.2%}".format(mAP))
    accs = np.mean(accs, axis=0)
    for i, k in enumerate(topk):
        write_text(sentence="  top-{:2d} = {:.2%}".format(k, accs[i]), fpath=osp.join(outsys_dir, 'os.txt'))
        # print("  top-{:2d} = {:.2%}".format(k, accs[i]))
