...
from models.resnet import build_convnext


class RFGCTCasNet(nn.Module):
    def __init__(self, cfg):
        super(RFGCTCasNet, self).__init__()

        backbone, box_head = build_convnext(arch='convnext_tiny',
                                            pretrained=True,
                                            freeze_layer1=True)
        box_head_2nd = deepcopy(box_head)
        box_head_3rd = deepcopy(box_head)
        ...
        rpn = RegionProposalNetwork(
            ...
            head=head,
            fg_iou_thresh=cfg.MODEL.RPN.POS_THRESH_TRAIN,   #0.7
            bg_iou_thresh=cfg.MODEL.RPN.NEG_THRESH_TRAIN,   #0.3
            batch_size_per_image=cfg.MODEL.RPN.BATCH_SIZE_TRAIN,  #256
            positive_fraction=cfg.MODEL.RPN.POS_FRAC_TRAIN,     #0.5
            pre_nms_top_n=pre_nms_top_n,      #train: 12000   test: 6000
            post_nms_top_n=post_nms_top_n,     #train: 2000   test: 300
            nms_thresh=cfg.MODEL.RPN.NMS_THRESH,   #0.7
        )

        faster_rcnn_predictor = FastRCNNPredictor(768, 2)

        box_roi_pool = MultiScaleRoIAlign(
            featmap_names=["feat_res4"], output_size=14, sampling_ratio=2
        )
        box_predictor = BBoxRegressor(768, num_classes=2, bn_neck=cfg.MODEL.ROI_HEAD.BN_NECK)
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        """
        original_image_sizes = [img.shape[-2:] for img in images]  #[450,800]
        images, targets = self.transform(images, targets)  #images_size: [843, 1500],  images_size: [1,3,864,1504]
        ...
        if targets is not None and not query_img_as_gallery:
            # query    query也没经过这个   是后面哪个query_box_feats=[]
            ...
            # gallery
            boxes, _ = self.rpn(images, features, targets)   ###推理阶段 (300,4)
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        features = self.backbone(images.tensors)     #features: feat_res4: [1,1024,58,76]
        boxes, rpn_losses = self.rpn(images, features, targets)   #proposals: [2000,4] proposal_loss：两个字典
       # _, detector_losses = self.roi_heads(features, proposals, images.image_sizes, targets)   #features: (1,1024,58,76) proposals: (2000,4)   image.images_size:  (900,1200) targets: {'img_name': 's.jpg', 'boxes':tensor([]),'labels':}
...
class CascadeRoIHeads(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        super(CascadeRoIHeads, self).__init__(*args, **kwargs)

        #RoI head
        self.use_diff_thresh = cfg.MODEL.ROI_HEAD.USE_DIFF_THRESH  # True
        self.nms_thresh_1st = cfg.MODEL.ROI_HEAD.NMS_THRESH_TEST_1ST  # 0.4
        self.nms_thresh_2nd = cfg.MODEL.ROI_HEAD.NMS_THRESH_TEST_2ND  # 0.4
        self.nms_thresh_3rd = cfg.MODEL.ROI_HEAD.NMS_THRESH_TEST_3RD  # 0.5
        self.fg_iou_thresh_1st = cfg.MODEL.ROI_HEAD.POS_THRESH_TRAIN  # 0.5
        self.bg_iou_thresh_1st = cfg.MODEL.ROI_HEAD.NEG_THRESH_TRAIN  # 0.5
        self.fg_iou_thresh_2nd = cfg.MODEL.ROI_HEAD.POS_THRESH_TRAIN_2ND  # 0.6
        self.bg_iou_thresh_2nd = cfg.MODEL.ROI_HEAD.NEG_THRESH_TRAIN_2ND  # 0.6
        self.fg_iou_thresh_3rd = cfg.MODEL.ROI_HEAD.POS_THRESH_TRAIN_3RD  # 0.7
        self.bg_iou_thresh_3rd = cfg.MODEL.ROI_HEAD.NEG_THRESH_TRAIN_3RD  # 0.7
        #regression head
        ...
        self.embedding_head_2nd = NormAwareEmbedding(featmap_names=["feat_res4", "feat_res5"],
                                                     in_channels=[384, 768], dim=embedding_dim)
        ...
        # evaluation
        self.eval_feat = cfg.EVAL_FEATURE  # 'concat'
        ...
        if self.training:
            if self.use_diff_thresh:
                self.proposal_matcher = det_utils.Matcher(
                    self.fg_iou_thresh_1st,  # 0.5
                    self.bg_iou_thresh_1st,  # 0.5
                    allow_low_quality_matches=False)
            boxes, _, box_pid_labels_1st, box_reg_targets_1st = self.select_training_samples(
                boxes, targets
            )     #proposals:(128,4),128个样本的位置，_表示128个proposal对应的第几个gt_boxes,proposals_pid_labels，表示这128个样本的label，（128，）[0,0,0,...,2331] proposal_reg_targets:128个proposal与gt_boxes的偏差

        # ------------------- The first stage ------------------ #
        box_features_1st = self.box_roi_pool(features, boxes, image_shapes)  #proposal_features:(128,1024,14,14)  ###推理阶段 (300,1024,14, 14)
        box_features_1st = self.box_head_1st(box_features_1st)    # 字典 {‘feat_res4’:(128,1024,1,1), 'feat_res5':(128,2048,1,1)}  ###推理阶段 {‘feat_res4’:(300,1024,1,1), 'feat_res5':(300,2048,1,1)}
        box_cls_scores_1st, box_regs_1st = self.box_predictor_1st(box_features_1st["feat_res5"])
       #预测的分类分数和回归，(128,2)  (128,8)    ###推理阶段 (300,2)  (300,8)
        if self.training:
            boxes = self.get_boxes(box_regs_1st, boxes, image_shapes)   #将预测偏移量与propsoal处理，得到（128，4)
            boxes = [boxes_per_image.detach() for boxes_per_image in boxes]   #detach()用于创建一个新张量，与原始张量共享相同的数据，但不再追踪梯度，
            if self.use_diff_thresh:  #True
                self.proposal_matcher = det_utils.Matcher(
                    self.fg_iou_thresh_2nd,   #0.6
                    self.bg_iou_thresh_2nd,   #0.6
                    allow_low_quality_matches=False)
        ...
        if not self.training and query_img_as_gallery:
            ...
            cws = False
            gt_box = [targets[0]["boxes"]]    ###query推理  (1,4) [915, 168.6, 1080, 636.933]
            gt_box_features = self.box_roi_pool(features, gt_box, image_shapes)   ###query推理 (1, 1024,14,14)
            gt_box_features = self.box_head_2nd(gt_box_features)   ###query推理 {'feat_res4: (1,1024, 1, 1),'feat_res5':(1, 2048,1 1}
            embeddings, _ = self.embedding_head_2nd(gt_box_features)    ###query推理  (1,256)
        ...
        # --------------------- The second stage -------------------- #
        box_features = self.box_roi_pool(features, boxes, image_shapes)  ## （128，1024，14，14）   ###推理阶段   (4, 1024, 14,14)假设上面一些列操作之后得到了4个proposals
        box_features = self.box_head_2nd(box_features)  ##字典：feat_res4:(128,1024,1,1)  feat_res5: (128,2048,1,1)    ###推理阶段 字典：feat_res4:(4,1024,1,1)  feat_res5: (4,2048,1,1)
        box_regs_2nd = self.box_predictor_2nd(box_features["feat_res5"])##(128,8)  ###推理阶段 (4,8)
        #box_embeddings, box_cls_scores = self.embedding_head(box_features)   #box_cls_scores:[128,]  box_embedding:[128,256]  ###推理阶段(4,256) (4,)
        ...
        if self.training:
            ...
            if self.use_diff_thresh:  #True
                self.proposal_matcher = det_utils.Matcher(
                    self.fg_iou_thresh_3rd,    #0.7
                    self.bg_iou_thresh_3rd,    #0.7
                    allow_low_quality_matches=False)
    ...
    def get_boxes(self, box_regression, proposals, image_shapes):
        ...
        """
        boxes_per_image = [len(boxes_in_image) for boxes_in_image in proposals]  #boxes_per_image:128
        pred_boxes = self.box_coder.decode(box_regression, proposals)  #(128,2,4) 将偏移量与proposal处理
        pred_boxes = pred_boxes.split(boxes_per_image, 0)  #在维度0上拆分为128个子张量 #（128，2，4）

        all_boxes = []
        for boxes, image_shape in zip(pred_boxes, image_shapes):
            boxes = box_ops.clip_boxes_to_image(boxes, image_shape)  #限制在图像内
            # remove predictions with the background label
            boxes = boxes[:, 1:].reshape(-1, 4)   #去掉背景的label  (128,4)
...
class NormAwareEmbedding(nn.Module):
    ...
    def __init__(self, featmap_names=["feat_res4", "feat_res5"], in_channels=[512, 1024], dim=256):
    ...
    def forward(self, featmaps):
        ...
        if len(featmaps) == 1:
            ...
            for k, v in featmaps.items():
                v = self._flatten_fc_input(v)
                outputs.append(self.projectors[k](v))    ###outputs:list为2，均为（128，128）
            embeddings = torch.cat(outputs, dim=1)    ####
            norms = embeddings.norm(2, 1, keepdim=True)
            embeddings = embeddings / norms.expand_as(embeddings).clamp(min=1e-12)
            norms = self.rescaler(norms).squeeze()   ###可以变成负的
            return embeddings, norms
