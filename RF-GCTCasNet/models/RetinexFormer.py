...
class RetinexFormer(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=32, stage=1, num_blocks=[1, 1, 1]):
        super(RetinexFormer, self).__init__()
        self.stage = stage
#将level:2->1  stage:3->1
        modules_body = [
            RetinexFormer_Single_Stage(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, level=1,
    ...
    def forward(self, image):
        ...
        out = self.body(image)
        # out = torch.split(out, 1, dim=0)    #变为[C,H,W]
...
class Denoiser(nn.Module):
    def __init__(self, in_dim=3, out_dim=3, dim=32, level=2, num_blocks=[2, 4, 4]):
    ...
    def forward(self, x, illu_fea):
        ...
        for (IGAB, FeaDownSample, IlluFeaDownsample) in self.encoder_layers:
          ...
          #  fea = FeaDownSample(fea)改
          #  illu_fea = IlluFeaDownsample(illu_fea)改

        # Bottleneck  改：bottleneck不要了
     #   fea = self.bottleneck(fea,illu_fea)

        # Decoder   改：decoder不要了
        for i, (FeaUpSample, Fution, LeWinBlcok) in enumerate(self.decoder_layers):
          #  fea = FeaUpSample(fea)  改
            fea = Fution(
                torch.cat([fea, fea_encoder[self.level - 1 - i]], dim=1))
           # illu_fea = illu_fea_list[self.level-1-i]改
...
class Illumination_Estimator(nn.Module):
    def __init__(
        ...
        self.depth_conv = nn.Conv2d(
            32, 32, kernel_size=5, padding=2, bias=True, groups=n_fea_in)
