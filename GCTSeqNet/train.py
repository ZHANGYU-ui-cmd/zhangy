...
def main(args):
    ...
    model = GCTSeqNet(cfg)
    # check_pt = 'output/epoch_29.pth'
    # model.load_state_dict(torch.load(check_pt)['model'])
    model.to(device)
    print('load success')
    # assert 0
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS):
        ...
        if (epoch + 1) % cfg.EVAL_PERIOD == 0 or epoch == cfg.SOLVER.MAX_EPOCHS - 1:
            ...
            evaluate_performance(
                ...
                query_loader,
                device,
                use_gt=cfg.EVAL_USE_GT,   #False
                use_cache=cfg.EVAL_USE_CACHE,  #false
                use_cbgm=cfg.EVAL_USE_CBGM,   #false
