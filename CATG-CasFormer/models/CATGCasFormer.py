...
class CATGCasFormer(nn.Module):
    def __init__(self, cfg):
        super(CATGCasFormer, self).__init__()

        backbone, _ = build_convnext(arch='convnext_tiny',
                                     pretrained=True,
                                     freeze_layer1=True)
        senet = GCT(num_channels=384)
        ...
        box_head = TransformerHead(
            cfg=cfg,
            trans_names=cfg.MODEL.TRANSFORMER.NAMES_1ST,  # ['scale1','scale2']
            kernel_size=cfg.MODEL.TRANSFORMER.KERNEL_SIZE_1ST,  # [(1,1),(3,3)]
            use_feature_mask=cfg.MODEL.TRANSFORMER.USE_MASK_1ST,  # False
        )
        box_head_2nd = TransformerHead(
            cfg=cfg,
            trans_names=cfg.MODEL.TRANSFORMER.NAMES_2ND,
            kernel_size=cfg.MODEL.TRANSFORMER.KERNEL_SIZE_2ND,
            use_feature_mask=cfg.MODEL.TRANSFORMER.USE_MASK_2ND,  # True
        )
        box_head_3rd = TransformerHead(
            cfg=cfg,
            trans_names=cfg.MODEL.TRANSFORMER.NAMES_3RD,
            kernel_size=cfg.MODEL.TRANSFORMER.KERNEL_SIZE_3RD,
            use_feature_mask=cfg.MODEL.TRANSFORMER.USE_MASK_3RD,  # True
        )
        ...
        self.local_senet = GCT(num_channels=384)
        self.local_cc = nn.Conv2d(384, 256, 1)
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        features_atten = self.senet(features['feat_res4'])  ###现在应该是[1, 384, 58, 76]
        features_atten = self.avg_pool(features_atten)
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            for sp in range(3):  ###遍历上、中、下
                for b in range(len(split_boxes)):
                    boxes_three[sp].append(split_boxes[b].transpose(0, 1)[sp])
            ...
            local_feature = torch.cat((self.local_fc_1(feat_medi[:, 0, :]), self.local_fc_2(feat_medi[:, 1, :]),
                                       self.local_fc_3(feat_medi[:, 2, :])), dim=-1)

            embeddings_2nd, _ = self.roi_heads.embedding_head_2nd(box_features_2nd, features_atten, batch_pro_nums,
                                                                  local_feature)
            box_features_3rd = self.roi_heads.box_head_3rd(box_features)
            embeddings_3rd, _ = self.roi_heads.embedding_head_3rd(box_features_3rd, features_atten, batch_pro_nums,
                                                                  local_feature)
            ...
            detections = \
            self.roi_heads(features, boxes, images.image_sizes, features_atten, targets, query_img_as_gallery)[0]
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        _, rcnn_losses, feats_reid_2nd, targets_reid_2nd, feats_reid_3rd, targets_reid_3rd = self.roi_heads(features,
                                                                                                            boxes,
                                                                                                            images.image_sizes,
                                                                                                            features_atten,
                                                                                                            targets)
...
class CascadedROIHeads(RoIHeads):
    ...
    def __init__(
        ...
        # feature mask
        self.use_feature_mask = cfg.MODEL.USE_FEATURE_MASK  # True
        self.feature_mask_size = cfg.MODEL.FEATURE_MASK_SIZE  # 4

        # Feature embedding
        # embedding_dim = cfg.MODEL.EMBEDDING_DIM   #256
        embedding_dim = 384
        ...
        self.reid_loss_2nd = OIMLoss(512, num_pids, num_cq_size, oim_momentum, oim_scalar)
    ...
    def forward(self, features, boxes, image_shapes, features_atten, targets=None, query_img_as_gallery=False):
        """
        Arguments:
        ...
        if not self.training and query_img_as_gallery:
            ...
            for sp in range(3):  ###遍历上、中、下
                for b in range(len(split_boxes)):  ####遍历每张图片
                    boxes_three[sp].append(
                        split_boxes[b].transpose(0, 1)[sp])  ###boxes_th
            ...
            embeddings, _ = self.embedding_head_2nd(gt_box_features, features_atten, batch_pro_nums,
                                                    local_feature)
        ...
        if boxes[0].shape[0] == 0:
            ...
            if self.eval_feat == 'concat':
                embeddings = torch.cat((gt_det_2nd["embeddings"], gt_det_2nd["embeddings"]),
                                       dim=1) if gt_det_2nd else torch.zeros(0, 1024)
        ...
        batch_pro_nums = [b.size(0) for b in boxes]
        box_embeddings_2nd, box_cls_scores_2nd = self.embedding_head_2nd(box_features, features_atten, batch_pro_nums,
                                                                         local_feature)
        ...
        if not self.training and query_img_as_gallery and self.eval_feat != 'stage2':
            ...
            embeddings, _ = self.embedding_head_3rd(gt_box_features, features_atten, batch_pro_nums, local_feature)
        ...
        if boxes[0].shape[0] == 0 and self.eval_feat != 'stage2':
            ...
            if self.eval_feat == 'concat':
                embeddings = torch.cat((gt_det_2nd["embeddings"], gt_det_3rd["embeddings"]),
                                       dim=1) if gt_det_3rd else torch.zeros(0, 1024)
            elif self.eval_feat == 'stage3':
        ...
        if not self.training:
            ...
            batch_pro_nums = [b.size(0) for b in boxes]
            box_embeddings_2nd, _ = self.embedding_head_2nd(box_features_2nd, features_atten, batch_pro_nums,
                                                            local_feature)

        box_features = self.box_head_3rd(box_features)
        box_regs_3rd = self.box_predictor_3rd(box_features["after_trans"])
        batch_pro_nums = [b.size(0) for b in boxes]
        box_embeddings_3rd, box_cls_scores_3rd = self.embedding_head_3rd(box_features, features_atten, batch_pro_nums,
                                                                         local_feature)
        ...
        if self.training:
            ...
            loss_rcnn_reid_2nd, feats_reid_2nd, targets_reid_2nd = self.reid_loss_2nd(box_embeddings_2nd,
                                                                                      box_pid_labels_2nd)
            loss_rcnn_reid_3rd, feats_reid_3rd, targets_reid_3rd = self.reid_loss_3rd(box_embeddings_3rd,
                                                                                      box_pid_labels_3rd)
...
class NormAwareEmbedding(nn.Module):
    ...
    def forward(self, featmaps, features_atten, batch_pro_nums, local_feature):
        ...
        if len(featmaps) == 1:
            ...
            features_atten = features_atten.flatten(start_dim=1)
            features_atten = self.fc(features_atten)
            local_feature = self.fclocal(local_feature)
            ...
            if list(local_feature.shape[:2]) == [sum(batch_pro_nums), 64]:
                embeddings = torch.cat((embeddings, local_feature), dim=1)
